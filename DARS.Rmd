...
df["dars_diagnosis"] <- sample (c('MDD', 'HC'), size = 20, replace = TRUE)

#add a column with fake sex (M, F, O)
df["dars_sex"] <- sample (c('F', 'M'), size = 20, replace = TRUE)

#add a column with fake age (60-8-)
df["dars_age"] <- sample (c(20:80), size = 20, replace = TRUE)
...
#hobbies
#hobbiesList_vars <- select_vars(names(df), contains('dars_hobbies_'), -contains('ratings')) #list of all the variables that match the string - exclude ratings
hobbiesList_idx <-  grep("dars_hobbies_", names(df)) #list of index of these vars - exclude ratings ---------------------------Still Includes ratings though------------------------

#hobbiesRatings_vars <- select_vars(names(df), contains('dars_hobbies_ratings_')) #list of all the variables that match the string 
hobbiesRatings_idx <-  grep("dars_hobbies_ratings_", names(df)) #list of index of these vars

foodsList_idx <-  grep("dars_foods_", names(df))#list of index of these vars
foodsRatings_idx <-  grep("dars_foods_ratings_", names(df)) #list of index of these vars

socialList_idx <-  grep("dars_social_", names(df))#list of index of these vars
socialRatings_idx <-  grep("dars_social_ratings_", names(df)) #list of index of these vars

sensoryList_idx <-  grep("dars_sensory_", names(df))#list of index of these vars
...
#fill in ratings
for (par in record_id) {
  vec <- darsRating_fn(sensory, foodsRatings_idx)
...
```{r}
list_vars <- c('dars_hobbies_ratings_01', 'dars_hobbies_ratings_02', 'dars_hobbies_ratings_03', 'dars_hobbies_ratings_04', 'dars_foods_ratings_01', 'dars_foods_ratings_02', 'dars_foods_ratings_03', 'dars_foods_ratings_04', 'dars_social_ratings_01', 'dars_social_ratings_02', 'dars_social_ratings_03', 'dars_social_ratings_04', 'dars_sensory_ratings_01', 'dars_sensory_ratings_02', 'dars_sensory_ratings_03', 'dars_sensory_ratings_04') 

df$rating_sum <- rowSums(df[, c('dars_hobbies_ratings_01', 'dars_hobbies_ratings_02')])
```
...
#####your code above ^^^ totally works, but we can do it in a slightly less redundant way! let me know what you think. :)
